    ...
    public static void main(String[] args) throws FileNotFoundException{
        ...
        for(Point[] pair: input){
            ...
            if(Math.abs(pair[0].x - pair[1].x) == Math.abs(pair[0].y - pair[1].y)){
                int y = pair[0].y;
                for(int i = pair[0].x; i !=  pair[1].x + (pair[1].x - pair[0].x)/Math.abs(pair[0].x - pair[1].x); i += (pair[1].x - pair[0].x)/Math.abs(pair[0].x - pair[1].x)){
                     result[i][y]++;
                     y += (pair[1].y - pair[0].y)/Math.abs(pair[0].y - pair[1].y);
                }
            }
        }
        int sum = 0;
